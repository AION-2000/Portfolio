...
export const GridScan: React.FC<GridScanProps> = ({
    ...
    useEffect(() => {
        ...
        try {
            ...
            console.error('GridScan initialization error:', err);
            return;
        }

        const onResize = () => {
            renderer.setSize(container.clientWidth, container.clientHeight);
            if (material) (material as any).uniforms.iResolution.value.set(container.clientWidth, container.clientHeight, renderer.getPixelRatio());
        ...
        const tick = () => {
            ...
            if (material) {
                (material as any).uniforms.uSkew.value.set(skew.x, skew.y);
                (material as any).uniforms.uTilt.value = tiltCurrent.current * tiltScale;
                (material as any).uniforms.uYaw.value = THREE.MathUtils.clamp(yawCurrent.current * yawScale, -0.6, 0.6);
                (material as any).uniforms.iTime.value = now / 1000;
            }
        ...
        return () => {
            ...
            if (material) (material as THREE.ShaderMaterial).dispose();
            if (quad && (quad as THREE.Mesh).geometry) ((quad as THREE.Mesh).geometry as THREE.BufferGeometry).dispose();
            if (composerRef.current) {
    ...
    useEffect(() => {
        ...
        if (m) {
            ...
            console.log('GridScan active uniforms update');
